...
class _PlayoffBracketPageState extends State<PlayoffBracketPage> {
  ...
  static const List<String> _kSeasons = ['Winter 2026', 'Summer 2026'];
  String _season = 'Winter 2026';
      ...
      pw.Page(
        build: (context) {
          return pw.Column(
            ...
            children: [
              pw.Text('$_season Playoff', style: pw.TextStyle(fontSize: 18)),
  ...
  Future<void> _showPlayersDialog(
    ...
    List<_BracketMatch> roundMatches,
    bool isLocked,
  ) async {
    if (isLocked) {
      ScaffoldMessenger.of(context).showSnackBar(
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    final occupiedIds = roundMatches
                        .where((other) => other.index != match.index)
                        .expand(
                          (other) => [
                            other.playerA?.playerId,
                            other.playerB?.playerId,
                          ],
                        )
                        .whereType<String>()
                    ...
                    if (selectedIds.any(occupiedIds.contains)) {
                      messenger.showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Jedan od odabranih igrača je već u drugom meču ove runde.',
                          ),
                        ),
                      );
                      return;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...matches.map(
            (match) =>
                _buildMatchCard(match, selectablePlayers, matches, isLocked),
          ),
        ],
      ),
    );
