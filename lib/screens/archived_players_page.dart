        ...
        body: TabBarView(
          children: [
            StreamBuilder<QuerySnapshot>(
              stream: _firestoreService.playerArchiveStream(),
              builder: (context, snapshot) {
                ...
                if (docs.isEmpty) {
                  return const Center(child: Text('Arhiva je prazna.'));
                }
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final played =
                        int.tryParse(stats['played']?.toString() ?? '0') ?? 0;
                    final wins =
                        int.tryParse(stats['wins']?.toString() ?? '0') ?? 0;
                    final losses =
                        int.tryParse(stats['losses']?.toString() ?? '0') ?? 0;
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Statistika: ${wins}W ${losses}L • $played mečeva',
                            ),
                            Text(
                              'Arhivirano: ${_formatStatusDate(archivedAt)}',
                            ),
                          ],
                        ),
                        isThreeLine: true,
            ...
            StreamBuilder<QuerySnapshot>(
              stream: _firestoreService.playersRawStream(),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }
                ...
                if (docs.isEmpty) {
                  return const Center(child: Text('Nema zamrznutih igrača.'));
                }
                              ...
                              children: [
                                ...
                                TextButton.icon(
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (confirm == true) {
                                      await _firestoreService.unfreezePlayer(
                                        docId,
                                      );
                                      if (context.mounted) {
