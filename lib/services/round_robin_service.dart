...
class RoundRobinService {
  ...
  List<ScheduledRound> generateWinterSchedule({
    ...
    return _generateSchedule(
      players: players,
      league: league,
      playDates: _winterPlayDates(seasonYear),
      cycleLabel: cycleLabel,
    );
  }

  List<ScheduledRound> generateSummerSchedule({
    required List<Player> players,
    required String league,
    required int seasonYear,
    String cycleLabel = 'Cycle 1',
    ...
    return _generateSchedule(
      players: players,
      league: league,
      playDates: _summerPlayDates(seasonYear),
      cycleLabel: cycleLabel,
    );
  }
  ...
  List<ScheduledRound> _generateSchedule({
    required List<Player> players,
    required String league,
    required List<DateTime> playDates,
    String cycleLabel = 'Cycle 1',
  }) {
  ...
  List<DateTime> _winterPlayDates(int seasonYear) {
    ...
    return _weekendDatesBetween(start, end);
  }

  List<DateTime> _summerPlayDates(int seasonYear) {
    final start = DateTime(seasonYear, 4, 16);
    final end = DateTime(seasonYear, 10, 14);

    return _weekendDatesBetween(start, end);
  }

  List<DateTime> _weekendDatesBetween(DateTime start, DateTime end) {
  ...
  List<List<_Pairing>> _buildRoundRobinRounds(List<Player> players) {
    final List<_RoundRobinEntry> entries = players
        .map(
          (p) => _RoundRobinEntry(id: p.id ?? '', name: p.name, isBye: false),
        )
        .toList();

    if (entries.length.isOdd) {
      entries.add(_RoundRobinEntry(id: '__BYE__', name: 'BYE', isBye: true));
    }
...
class _RoundRobinEntry {
  ...
  _RoundRobinEntry({required this.id, required this.name, required this.isBye});
}
...
class _Pairing {
  ...
  _Pairing({required this.player1, required this.player2});
}
